<template>
  <div class="bg-secondary text-black flex flex-col flex-1 items-center">
    <div class="bg-secondary flex-1 flex flex-col w-full pt-6 overflow-y-auto">
      <div
      class="grid md:[grid-template-columns:repeat(auto-fill,_minmax(17rem,3fr))] [grid-template-columns:repeat(auto-fill,_minmax(6rem,3fr))] gap-7 justify-center items-center w-full px-10 mb-10"
      >
        <ProductCard
          v-for="(item, index) in this.otherStoreStore?.otherStore?.products"
          :id="item._id"
          :key="item.title"
