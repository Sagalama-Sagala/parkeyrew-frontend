<template>
  <div class=" bg-secondary min-h-screen flex flex-col gap-2 overflow-hidden ">
    <div class="bg-white text-black pt-20 flex justify-center items-center ">
      <div class="border-b-[1px] border-b-black text-black flex md:gap-[10rem] gap-4 md:text-[1.3rem] text-[1.2rem] w-full mx-4 justify-center flex-wrap px-5 pb-3">
        <div class="flex flex-col gap-5 w-[40rem] "> 

          <div class="flex flex-wrap gap-2 bg-black rounded-xl">
            <div class="relative   ">
              <img :src="ProductImage[selectedImageIndex]" class="w-[40rem] h-[32rem]   object-contain "  /> 
              <img v-if="productData.isRecommend" :src="Recommend" class="w-[4rem]  absolute top-0 right-3  "  >
            </div>
          </div>

          <div class="flex justify-start gap-3 overflow-x-auto pb-3 overflow-y-visible ">
            <img 
            v-for="(imageUrl, index) in ProductImage" 
            :key="index" 
            :src="imageUrl" 
            class="h-[8rem] w-[11rem] border-grey pt-1 rounded-xl border-[1px]" 
            :class="{ ' blur-sm border-2 border-black': selectedImageIndex === index }"
        ...
        <div class=" w-[30rem] flex flex-col md:gap-12 gap-7 "> 
          ...
          <div class="flex justify-between md:flex-row flex-col "> 
            <div class="flex flex-row md:flex-col md:justify-end justify-between md:text-[1.5rem] font-bold"> 
              <h1>ชื่อ : {{productData.name}}</h1> 
              <h1>ราคา : {{productData.price}} บาท</h1>
            </div>

            <div class="flex md:flex-col md:justify-center justify-between items-center"> 
              <div class="flex gap-2"> 
                  <img :src="Heart" class="border-[2px] border-grey rounded-xl flex justify-center items-center p-1 hover:bg-secondary md:w-14 w-10">
                  <img :src="Sharearrow" class="border-[2px] border-grey rounded-xl flex justify-center items-center hover:bg-secondary md:w-14 w-10 p-2">
              </div> 
          ...
          <div class="flex  items-center border-[1px] border-[#111111] px-3 rounded-2xl h-[5rem] gap-3   justify-between  ">
            ...
            <div class="flex gap-3 ">  
              <div class="flex  border-[1px] border-[#393838] md:border-black w-[4rem] md:w-[6rem] h-[4rem] rounded-full md:rounded-xl justify-center items-center gap-2 hover:bg-secondary hover:cursor-pointer"> 
  ...
  data()
  {
    return{
      ...
      productData:
      {
        ...
        name:"กางเกงเป็ด",
        price:1000,
        detail:"รายละเอียดสินค้ารายละเอียดสินค้ารายละเอียดสินค้ารายละเอียดสินค้ารายละเอียดสินค้ารายละเอียดสินค้ารายละเอียดสินค้า",
        isRecommend:true,
        remain:0,
        brand:"nike",
        color:"แดง",
        ...
        sellerName:"ชาบู",
        sellerImage:"https://ichef.bbci.co.uk/news/976/cpsprodpb/16620/production/_91408619_55df76d5-2245-41c1-8031-07a4da3f313f.jpg",
        rating:4.6
